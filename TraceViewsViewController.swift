...
class TraceViewsViewController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        KiteController.shared.settings.tetherLength.asObservable()
            .bind { radius in self.freeView.domeRadius = radius }
            .disposed(by: bag)

        KiteController.shared.settings.tetherLength.asObservable()
            .bind { radius in self.xyView.domeRadius = radius }
            .disposed(by: bag)

        // Coordinate axes

        let axes = [(e_x, NSColor.red), (e_y, .green), (e_z, .blue)].map { ArrowDrawable(vector: 5*$0, color: $1) }
        axes.forEach(xyView.add)
        axes.forEach(freeView.add)
        ...
        let cBall = BallDrawable(color: .green)
        xyView.add(cBall)
        freeView.add(cBall)

        cPoint
            .bind { pos in cBall.position = pos}
            .disposed(by: bag)

        let piCircleDrawable = CircleDrawable()
        xyView.add(piCircleDrawable)
        freeView.add(piCircleDrawable)
